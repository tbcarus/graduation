...
public interface DishRepository extends JpaRepository<Dish, Integer> {
    ...
    @Query ("SELECT d FROM Dish d WHERE d.restaurant.id =: restaurantId ORDER BY d.restaurant.name, d.name DESC")
    List<Dish> getAllByRestaurantId(@Param("restaurantId") int restaurantId);

    @Query("SELECT d FROM Dish d WHERE d.inputDate =: date ORDER BY d.name DESC")
